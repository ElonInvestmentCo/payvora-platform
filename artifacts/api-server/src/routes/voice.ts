...
const voiceJson = (v: { id: string; name: string; description: string | null; category: string; favorite: boolean; referenceText: string | null; lastUsedAt: Date | null; createdAt: Date }) => ({
  id: v.id, name: v.name, description: v.description, category: v.category, favorite: v.favorite,
  referenceText: v.referenceText, lastUsedAt: v.lastUsedAt?.toISOString() ?? null, createdAt: v.createdAt.toISOString(),
});
...
router.get("/voice/capabilities", (_req, res) => {
  ...
  res.json({
    ...
    supportedControls: ["speed", "pitch", "energy", "emotion"],
    unsupportedControls: [
