...
export default function TextToSpeech({ onNavigate }: { onNavigate?: (tab: string) => void }) {
  ...
  useEffect(() => {
    ...
    getSettings().then(s => {
      ...
      if (typeof s.emotion === 'string') setEmotion(s.emotion)
  ...
  useEffect(() => {
    ...
    const handle = setTimeout(() => {
      void saveSettings({ speed, pitch, emotion, naturalPauses, outputFormat, sampleRate }).catch(() => undefined)
    }, 600)
    return () => clearTimeout(handle)
  }, [speed, pitch, emotion, naturalPauses, outputFormat, sampleRate])
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      if (emotion) settings.emotion = emotion
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
            {[
              ...
              { label: 'Whisper', snippet: ' [whisper] ', title: 'Whisper the following speech', icon: <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2a3 3 0 0 0-3 3v7a3 3 0 0 0 6 0V5a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/></svg> },
            ].map(b => (
              <button key={b.label} title={b.title} onClick={() => insertAtCursor(b.snippet)}
                style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, fontWeight: 500, color: C.accent, background: C.accentLight, border: 'none', borderRadius: 20, padding: '6px 12px', cursor: 'pointer', transition: 'background 0.15s' }}
        ...
        <div style={{ background: C.white, border: `1px solid ${C.border}`, borderRadius: 16, padding: 20 }}>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 12, marginBottom: 20 }}>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              ...
              <SelectDropdown value={emotion} options={EMOTION_OPTIONS} onChange={setEmotion} label="Speaking style" />
            </div>
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
